...
// API Configuration - Direct call to Microburbs API (no backend needed!)
const API_URL = "https://www.microburbs.com.au/report_generator/api/suburb/properties";
const API_HEADERS = {
...
// Load Sample Data (from response.json as fallback)
...
// Handle Search - Direct API call (no Flask backend needed!)
async function handleSearch() {
    ...
    try {
        ...
        // Make direct API call to Microburbs
        ...
        if (!currentData || currentData.length === 0) {
            showError('No properties found for the selected criteria. Try a different suburb or property type.');
            hideLoading();
            return;
        }
        ...
        if (suburb.toLowerCase().includes('belmont')) {
            console.log('API failed, loading sample data as fallback...');
            await loadSampleDataFromFile();
        } else {
            showError(`Unable to fetch data: ${error.message}. Please try again or use "Belmont North" to view sample data.`);
        }
        hideLoading();
    }
}

// Load sample data from response.json file
async function loadSampleDataFromFile() {
    try {
        const response = await fetch('response.json');
        const data = await response.json();
        currentData = data.results;
        displayResults(currentData, 'Belmont North (Sample Data - API Unavailable)');
    } catch (error) {
